...
$max \chi^2$计算了规则的$\chi^2$的上限，其他设置都是固定的。
对于每一组规则，$\chi^2$的权重定义为

$$
\sum \frac{ $\chi^2$ $\chi^2$ } { $max \chi^2$ }
$$

正如所看到的那样，我们用$\chi^2$的值在它的上限（即，$max \chi^2$）中所占的比重来平衡$\chi^2$对少数类别的偏差。
请注意，从理论上讲，很难验证衡量标准对规则组强度的合理性或效果。
相反，我们根据经验探索测量的效果，并且根据我们的实验结果，加权$\chi^2$值是我们能够计算出的一组好的候选测量公式中的最佳值。

## 5 实验结果和性能研究

为了评估CMAR的准确性，效率和可扩展性，我们进行了广泛的性能研究。
在本节中，我们将比较CMAR与两种流行的分类方法：CBA [9]和C4.5 [10]的实验结果。
结果表明，CMAR在平均准确性，效率和可扩展性方面均优于CBA和C4.5。

所有实验均在运行Microsoft Windows / NT的具有128M主内存的600MHz Pentium PC上进行。
CBA和C4.5分别由其作者实施。
在实验中，三种方法的参数设置如下。

所有C4.5参数均为默认值。我们测试了C4.5决策树方法和规则方法。
由于规则方法具有更高的准确性，因此我们仅报告规则方法的准确性。

对于CBA，我们将支持阈值设置为1%，将置信度阈值设置为50%，并禁用规则数量限制。其他参数保留默认值。

对于CMAR，将支持和置信度阈值设置为与CBA相同。数据库覆盖率阈值设置为4，置信度差异阈值设置为20%。

运行时的所有报告均包括CPU时间和I / O时间。

我们从UCI机器学习存储库中测试26个数据集。
我们使用C4.5的shuffle实用程序来随机打乱数据集。
此外，我们采用了CBA用来离散化连续属性的相同方法。

从表中可以看出，CMAR的准确性优于C4.5和CBA。
此外，在26个的数据集中，CMAR的有13个达到了精度最高。
换句话说，CMAR赢得了50％的测试数据集。
在某些数据集中，例如Lymph，CMAR以5％的精度赢得了第二名。

CMAR中有两个重要参数，数据库覆盖阈值和置信度差阈值。
如前所述，这两个阈值控制选择用于分类的规则的数量。

通常，如果规则集太小，则可能会错过一些有效规则。
另一方面，如果规则集太大，则训练数据集可能会过拟合。
因此，我们需要测试两个阈值关于分类准确性的灵敏度。

例如，我们在UCI机器学习数据库存储库中的Sonar数据集上测试了不同的数据库覆盖阈值。
结果如图5所示，其中置信度差阈值设置为0。
另一方面，我们在Sonar数据集上测试不同的置信度差异阈值。
结果如图6所示，其中数据库覆盖率阈值设置为1。

从图中可以看出，在两条曲线的中间都达到了精度的峰值。
也就是说，两个阈值都有最佳设置。
但是，根据我们的实验结果，似乎无法预先确定最佳阈值。
幸运的是，两条曲线都很平坦。这意味着精度对两个阈值不是很敏感。

CR-tree是用于存储规则的紧凑结构。
为了测试CR树的有效性，我们在大型测试数据集上比较了CBA和CMAR的主要内存使用情况。
结果示于表4。

请注意，在此实验中，我们禁用了CBA中规则数量的限制。
在这种情况下，CBA和CMAR会生成分类所需的所有规则，因此可以在公平的基础上进行比较。
从表中可以看到，平均而言，CMAR可以节省77.12％的主内存使用量。

可以从两个方面解释主存储器使用量的节省。

首先，CMAR使用CR-tree。 
CR-tree的紧凑性在存储大量规则（其中可以共享规则中的许多项）方面带来了可观的收益。

另一方面，CR-tree也是规则的索引结构。
在将规则插入到CR-tree中之前，CMAR会检查树中是否存在通用规则或某些更特定的规则。
如果是这样，则立即进行相关修剪。这种修剪技术也有助于节省主内存。

为了测试CMAR的可伸缩性，我们在六个数据集上比较了CBA和CMAR的运行时间。
结果如图5所示。再次，我们禁用了CBA中规则数量的限制。
在实验中，CBA在I / O上花费了很大一部分运行时。

从表中可以看出，在许多情况下，CMAR比CBA快。
请注意，我们用于测试的机器的主内存容量相对较小（128M）。
如果有更多的主内存可用，则CBA和CMAR的运行速度都有望大大提高。

## 6 结论

在本文中，我们研究了关联分类中的两个主要挑战：
（1）处理大量挖掘的关联规则的效率，以及
（2）预测具有高分类精度的新类别标签的效率。
我们提出了一种新颖的关联分类方法CMAR，即基于多重关联规则的分类。
该方法具有以下几个显着特征：
（1）基于在多个关联规则上执行的加权$ \chi^2 $分析进行分类，从而提高了整体分类的准确性
（2）根据置信度，相关性和数据库覆盖范围有效修剪规则，并且
（3）通过扩展有效的频繁模式挖掘方法，FP增长，构造与类分布相关的FP-tree来实现其效率；
以及应用CR树结构有效地存储和检索挖掘的关联规则。

我们在UCI机器学习数据库存储库中的26个数据库上进行的实验表明，与CBA和C4.5相比，CMAR是一致的，在对各种数据库进行分类时效率很高，并且平均分类准确度更高，并且比其他关联方法更有效，更可扩展分类方法。
